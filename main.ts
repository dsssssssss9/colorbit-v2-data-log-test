...
datalogger.setColumns(["Volume", "Magnetic"])
Logging = false
loops.everyInterval(1000, function () {
    if (Logging) {
        basic.showIcon(IconNames.Heart)
        datalogger.logData([datalogger.createCV("Volume", input.soundLevel()), datalogger.createCV("Magnetic", input.magneticForce(Dimension.X))])
        basic.showIcon(IconNames.Heart)
    }
})
